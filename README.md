...
- **Agent Window（開発中）** — エディタタブとして開く大画面エージェントGUI。プロジェクト名/パス表示、設定導線、モデル用途切替、日本語の進行表示、AIタイトル付きタスク作成、タスク削除、履歴、Agent送信、承認/拒否カード、同一タスク二重実行排他、軽量ファイルツリー、localhostプレビュー、@ファイルメンションまで接続済み。外部URLはSimple Browserで開く
...
- **Agent Window (in development)** — opens a large editor-tab agent GUI with project context, settings, model intent switching, Japanese progress status, AI-titled task creation, task deletion, history, agent sends, approve/reject cards, same-task run exclusion, lightweight file tree, localhost preview, and file mentions wired; external URLs open in Simple Browser
