...
- **Agent Window進行表示と内部REMINDER漏れ修正**:
  - **`webview/src/agentWindowProgress.ts` / `webview/src/AgentWindow.tsx` / `webview/src/agent-window.css`**: Agent Windowヘッダー下に `コードレビューについて思考中...` / `回答を組み立て中...` / `承認が必要な操作があります` などの日本語進行ステータスを表示
  - **`src/backend/toolResultReminder.ts` / `src/backend/tools.ts`**: `attempt_completion` の最終出力には内部用 `[REMINDER: ...]` を付けないよう修正
  - **`webview/src/agentWindowMessageText.ts`**: 既に保存済みの履歴に内部REMINDERが残っていてもAgent Window表示時は除去する
- **0.8.1 配布用バージョン更新**:
...
### 2026-07-02
- **不要な `activationEvents` 宣言を削除**:
  - **`package.json`**: `activationEvents: ["onView:torii-view"]` を削除。VS Code 1.74以降は `contributes.views` からアクティベーションイベントが自動推論されるため、明示指定はVS Code側で「コントリビューション宣言から自動生成される」という警告の対象になっていた。`engines.vscode` は `^1.85.0` のため要件を満たす
