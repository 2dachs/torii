...
1. **Markdownレンダリング不在** — `App.tsx` の `message-content` がプレーンテキスト表示。コードブロック・ハイライトが生テキストのまま。競合（Cline/Cursor/Copilot Chat）は全て対応済みで、第一印象の品質差が最大。
   - 対応案: `react-markdown` + `rehype-highlight`。コードブロック単位のコピー/Applyボタンで差別化も可能
2. **チャットモードに応答ストリーミングがない** — `callOpenAICompatible()` が全文一括返却。長い回答で「考え中…」を30秒以上見せる。Agentモード（SSE済み）より無料側が遅く感じる逆転現象
3. **初回オンボーディング不在** — キー未設定で送信→エラー→設定画面の流れ。「コストが怖い初心者」がターゲットなのに最初の体験が失敗から始まる
   - 対応案: 初回起動ウィザード（Ollama無料スタートを第一選択肢に。`ollamaSetup.ts` 流用可）
4. **エージェント変更のrevert手段がない（チェックポイント不在）** — Apply後に戻せない。最小対応: write_file前の旧内容を保存し「元に戻す」ボタン。本格対応: タスク単位チェックポイント（Cline方式）
   - 対応済み: `write_file` / `replace_in_file` 適用時に旧内容checkpointを保存し、Agent進捗UIに「元に戻す」ボタンを表示。既存ファイルは旧内容へ復元、新規作成ファイルは削除する。
5. **run_command承認に許可リストがない** — 毎回承認が必要で自律性が半減。「今後 `npm test` は自動許可」の3択化を推奨（危険パターンは許可リストにあっても必ず確認）
   - 対応済み: run_command承認カードに「今後も許可」を追加。`torii.commandAllowlist` に完全一致コマンドを保存し、危険コマンドはallowlist登録済みでも `commandGuard` でブロックする。

### UX問題点（優先度: 中）

6. **モデル設定の概念過多** — 「現在のモデル」「メイン」「サブ」「上位再実行×2」「自動ルーティング」が並存。設定画面に実質同じ設定UIが2箇所（現在のモデル / メインモデル）。v0.2.5〜0.2.8の保存バグ4連発の根本原因でもある。ユーザー向け概念を「使用モデル」「節約モデル」の2つに集約すべき
   - 対応済み: 設定UIの重複した「現在のモデル / メインモデル」を「使用モデル」に一本化し、サブモデル表記を「節約モデル」に変更。内部設定キーは互換性維持のため `main*` / `sub*` を継続利用。
7. **@ファイルメンション・選択範囲送信がない** — コンテキスト指定がアクティブエディタ全文添付のみ。スラッシュサジェストの仕組みを流用して `@` ファイルサジェスト追加可能
8. **タスクのリネーム・削除・検索がUIにない** — `storage.ts` に `deleteTask` 実装済みだがUIから呼べない
9. **履歴クリア🗑️が確認なしで全削除** — `provider.ts` で即実行。破壊的操作に確認ダイアログなし
10. **エラー後のリトライ導線がない** — 再試行ボタン・レート制限時のフォールバック提案なし
11. **run_commandタイムアウト120秒固定** — `npm install` 等で失敗。devサーバー等の常駐プロセス非対応
12. **コンテキスト使用量の常時表示がない** — 80%警告のみで、超過時は古いメッセージが黙って消える。ステータスバーにトークンゲージを
13. **`.torii-todo.md` がリポジトリを汚す** — システムプロンプトでワークスペース直下に書かせている。コミット混入事故リスク。ツール化してUI側チェックリスト表示に変えるべき
14. **read_fileキャッシュがユーザー編集と競合** — `tools.ts` のキャッシュがタスク中保持され、ユーザーがエディタで保存した変更を無視。mtimeチェックで破棄を
...
①Markdownレンダリング → ②チャットストリーミング → ③オンボーディング（ユーザー獲得・継続率に直結し実装リスクが低い順）。Codex化はコア抽出（Step 1）が拡張本体のリファクタにもなるため、UX改善と並行可能。
