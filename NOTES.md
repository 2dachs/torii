...
- 予算バーの表示を数値スナップショット化し、累計コストの表示ズレを修正済み
- 初回オンボーディングを追加済み
- タスクのリネーム・削除・検索 UI を追加済み
- 履歴クリア時の確認ダイアログを追加済み
- エラー後のリトライ導線を追加済み
- `run_command` 承認に allowlist を追加済み
- `run_command` のタイムアウト可変化を追加済み
- `read_file` キャッシュとユーザー編集の競合を軽減済み
- コンテキスト使用量の常時表示を追加済み
- コマンドガードを強化済み
- `@` ファイルメンション / 選択範囲送信を追加済み
- モデル設定の概念を整理済み
...
12. **read_fileキャッシュがユーザー編集と競合** — `tools.ts` のキャッシュがタスク中保持され、ユーザーがエディタで保存した変更を無視。mtimeチェックで破棄を
