...
export interface ModelConfig {
  id: string;
  provider: 'openrouter' | 'sakana' | 'openai' | 'anthropic' | 'google' | 'local';
  displayName: string;
  modelSlug: string;
...
export function calculateCost(inputTokens: number, outputTokens: number, inputPrice: number, outputPrice: number): number {
...
export function routeMessage(args: {
  ...
  return { mode: args.mode, selectedModel, estimatedInputTokens, estimatedOutputTokens, estimatedCost, requiresConfirmation, reason };
}
